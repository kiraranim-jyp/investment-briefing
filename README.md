...
- DART corpCode.xml(11만+건)은 첫 요청(콜드 스타트) 시 다운로드+파싱 비용이 있어 즉시조회 응답이 느려질 수 있음 — 같은 서버리스 인스턴스가 살아있는 동안은 메모리 캐시로 재사용되지만, 인스턴스가 새로 뜨면 다시 비용 발생. Redis에 파싱 결과를 캐싱하면 근본적으로 해결 가능 (다음 단계)
